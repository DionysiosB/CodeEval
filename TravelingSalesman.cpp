...
void TSP(char* inputName) {
	...
	if(inputFile){		
		while(getline(inputFile,line)){	
			...
			currentPoint=splitCSV(line,',');Xlist.push_back(100000.0*currentPoint[0]);Ylist.push_back(100000.0*currentPoint[1]);
		}
		...
		for(unsigned row=0;row<numPoints;++row){
			...
			for(unsigned col=row+1;col<numPoints;++col){
				distanceMatrix[row][col]=pointDistance(Xlist[row],Ylist[row],Xlist[col],Ylist[col]);
		...
		vector<int> minPerm;
		float minDistance=100000000; //A large Number
		float pathDistance;
